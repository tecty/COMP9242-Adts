...
{
    printf("=> start %lu\tsize %lu\tcrrt: %p\tType:%u\n", region->start,
        region->size, region->crrt, region->type);
}
...
{
    struct AddressRegion_Region_s arrs = { .type = type,
        .size = (uint64_t)size,
...
{
    return (arrs->start <= (uint64_t)vaddr
...
{
    // struct AddressRegion_Region_s * arrs = AddressRegion__getRegionByPtr(ar,
    // index); if (arrs == NULL) return false; valid region to resize
...
void* AddressRegion__unmap(AddressRegion_t ar, void* start, size_t size)
{
    ...
    // not found the region
    if (context.found == NULL || context.found->type != MMAP
    ...
    DynamicArr__del(
        ar->regions, DynamicArr__getIndexByPtr(ar->regions, context.found));
    return ret;
}
