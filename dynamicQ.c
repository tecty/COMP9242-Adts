...
void DynamicQ__enQueue(DynamicQ_t dq, void* data)
{
    if (dq->alloced == dq->length - 2) {
        ...
        if (dq->tail < dq->head) {
            ...
            memcpy(dq->item_arr + (dq->length / 2) * dq->item_size,
                dq->item_arr, dq->tail * dq->item_size);
            dq->tail += dq->length / 2;
        }
    }
    dq->alloced++;
...
bool DynamicQ__isEmpty(DynamicQ_t dq)
{
    ...
    //     dq->tail, dq->alloced, dq->length); assert(dq->head == dq->tail);
    // }
    return dq->alloced == 0;
}
...
size_t DynamicQ__getAlloced(DynamicQ_t dq) { return dq->alloced; }
    ...
    DynamicQ_t dq, dynamicQ_compare_t cmp, void* private_data)
{
    ...
    while ((data = DynamicQ__first(dq)) && data != NULL) {
        // enqueue to new
        if (cmp(data, private_data))
            DynamicQ__enQueue(new, data);
        DynamicQ__deQueue(dq);
    }
    DynamicQ__free(dq);
    return new;
...
uint64_t sum;
void sumUp(void* data) { sum += *(uint64_t*)data; }
